<template>
  <section :class="isOpen ? 'w-[40%]': 'w-0'"
           class="flex flex-col h-full shrink-0 z-10 overflow-hidden border-white transition-all ease-in-out btn-bg border-r border-white lg:pt-[100px]">
    <div class="flex flex-col px-12 overflow-y">
      ...
      <div v-for="plant in plants" class="flex justify-between border-b border-white py-4">
        <span>{{ plant.name }} x{{ plant.number }}</span>
    ...
    <div class="flex justify-between mt-auto font-title font-bold text-16 lg:text-20 border-t border-white px-12 py-10">
